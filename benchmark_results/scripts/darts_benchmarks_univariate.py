...
dirname = f"darts_benchmarks_univariate_granularity_{pts_per_period}"
...
def run_forecast_benchmark(model_name, traj_train, all_hyperparameters, forecast_length, gpu_params, equation_name, dirname=None):
    ...
    for i in range(len(traj_train)):
        ...
        try:
            if model_name == "NVAR":
                model = import_model_by_name(model_name, 1, **all_hyperparameters)
                all_pred2 = list()
                for j in range(train_data.shape[1]):
                    model.fit(np.copy(train_data)[:, j][:, None])
                    y_val_pred = model.predict(forecast_length).squeeze().copy()
                    all_pred2.append(y_val_pred.copy())
                all_pred2 = np.array(all_pred2)
                y_val_pred = all_pred2.T
            else:
                model = import_model_by_name(model_name, **all_hyperparameters)
                all_pred2 = list()
                for j in range(train_data.shape[1]):
                    y_train_ts = TimeSeries.from_values(train_data.copy()[:, j][:, None])
