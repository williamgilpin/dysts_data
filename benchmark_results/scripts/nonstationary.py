...
dirname = f"nonstationary_context_{context_length}_granularity_{pts_per_period}"
...
def damp_trajectory(n, final_frac=0.01):
    """
    Return an exponentially decaying curve between 1 and final_frac
    """
    t = np.arange(n)
    damping_factor = -np.log(final_frac) / (n - 1)
    decay = np.exp(-damping_factor * t)
    return decay

shift_vals = np.linspace(.01, 1, 12)
...
# for model_size in ["tiny", "mini", "small", "base", "large"]:
    ...
    for shift in shift_vals:
        ...
        envelope = damp_trajectory(traj_test.shape[1], final_frac=shift)

        traj_test = np.copy(traj_test0) * envelope[None, :, None]
        traj_train = traj_test[:, :training_length] ## Training data for standard models                                     
        traj_test_context = traj_test[:, training_length - context_length:training_length]
        ic_test = traj_test_context[:, -1] # Last seen point                                                                 
        traj_test_forecast = traj_test[:, training_length:]
        save_str_reference = f"forecast_{eq.name}_granularity{pts_per_period}_true_chronos_shift{shift}"
        ...
        save_str_reference2 = f"context_{eq.name}_granularity{pts_per_period}_true_chronos_context_shift{shift}"
        ...
        save_str = f"forecast_{eq.name}_{model_size}_granularity{pts_per_period}_shift{shift}"
