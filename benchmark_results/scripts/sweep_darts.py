...
dirname = f"darts_contextsweep_context_{context_length}_granularity_{pts_per_period}"
...
def run_forecast_benchmark(model_name, traj_train, all_hyperparameters, forecast_length, 
                           gpu_params, equation_name, context_length=1, dirname=None):

    all_pred = list()
#     print("run input shape", traj_train.shape)
    for i in range(len(traj_train)):
        print(i, flush=True)
        train_data = np.copy(traj_train)[i]
        try:
            if model_name == "NVAR":
                model = import_model_by_name(
                    model_name, 
                    traj_train.shape[-1], 
                    **all_hyperparameters
                )
            ...
            y_val_pred = np.nan * np.ones((forecast_length, train_data.shape[1]))
        
        y_val_pred = np.array(y_val_pred.tolist())
#         print("run middle shape", y_val_pred.shape)
        all_pred.append(np.copy(y_val_pred))
    
    all_pred = np.array(all_pred)
#     print("run output shape", all_pred.shape)
    save_str = f"forecast_{equation_name}_{model_name}_granularity{pts_per_period}_context{context_length}"
...
for context_length in context_lengths:
    ...
    lookback_candidates = [5, 10,
                            min(context_length // 4, 25), 
                            min(context_length // 2, 50), 
                            min(context_length, 100)
                            ]

    model_name = "NBEATS"
    print(model_name, flush=True)
    hyperparameters = {"output_chunk_length": 1}
    hyperparameter_candidates = {"input_chunk_length": lookback_candidates}
    start = time.perf_counter()
    hyperparameters = cross_validate_hyperparameters(model_name, traj_test_context, 
                                                     hyperparameters, hyperparameter_candidates, gpu_params)
    middle = time.perf_counter()
    run_forecast_benchmark(
        model_name, traj_test_context, hyperparameters, 
    ...
    inference_time = finish - middle
    
    save_str = f"timings_{model_name}_granularity{pts_per_period}_context{context_length}"
    save_str = os.path.join(dirname, save_str)
    with open(save_str, 'a') as file:
        file.write(f"Train Time: {train_time}, Inference Time: {inference_time}\n")
